...
<cast:emailPageContainer >
  ...
  <jsp:attribute name="messageBody">
    ...
    <dsp:test var="objectSize" value="${paymentGroups}" />
    <c:choose>
      <c:when test="${objectSize.size>1}">    
        <dsp:getvalueof var="paymentMethod" value="mix"/>
      </c:when>
      <c:when test="${objectSize.size==1}"> 
            ...
            <hr style="width:100%; height :1px; color : #e2e2e2;">
              ...
              <p>
                ...
                <fmt:message key="email.confirmation.common.summary.payment.mode">
                  <fmt:param>
                    <c:choose>
                      <c:when test="${paymentMethod=='Cadeau' || paymentMethod=='cadeau' }">
                        <fmt:message key="email.confirmation.common.summary.payment.mode.gc"/>
                      </c:when>
                      <c:otherwise>
                        <fmt:message key="email.confirmation.common.summary.payment.mode.cbgc"/>
                      </c:otherwise>
                    </c:choose>
                  </fmt:param>
                </fmt:message>
