<dsp:page>
  ...
  <dsp:getvalueof var="icludeTrailParams" param="icludeTrailParams"/>
  ...
  <dsp:droplet name="/com/castorama/search/droplet/AjouterFacetValuesFinder">
    <dsp:param name="currentAjouterFacetId" param="ajouterFacetId"/>
    <dsp:param name="currentAjouterFacetTrail" value="${ajouterTrail }"/>
    <dsp:param name="previousTrail" value="${trailVar }"/>
    <dsp:param name="skipValuesAlreadyInTrail" value="true"/>
    <dsp:oparam name="output">
      <dsp:droplet name="/com/castorama/search/droplet/CastFacetValuesDroplet">
        ...
        <dsp:oparam name="oparam">
          ...
          <dsp:droplet name="/com/castorama/search/droplet/AjouterValuesCombiner">
                ...
                <dsp:oparam name="false">
                  ...
                  <c:choose>
                    <c:when test="${not empty containerVar && fn:contains(containerVar, '?')}">
                      <c:set var="actionContainerVar" value="&formid=${formid }&osearchmode=${osearchmode}"/>
                    </c:when>
                    <c:otherwise>
                       <c:set var="actionContainerVar" value="?formid=${formid }&osearchmode=${osearchmode}"/>
                    </c:otherwise>
                  ...
                  <dsp:getvalueof var="previousTrail" param="trail"/>
                  
                  <dsp:form action="" method="post" iclass="ajouterListForm${ajouterFacetId}">
                    ...
                    <input type="hidden" name="previousTrail" value="${previousTrail}"/>
                    ...
                    <script language="javascript">
                      var rootFacet = "";
                      var actionForForm="${containerVar}";
                      var inputsFacetValues="${urlTrail}".replace(/\|/g,'_');
                      var inputsPageUrlForFacets="${url}";
                      var actionContainerVar="${trailVar}";
                    </script>
                    ...
                    <dsp:droplet name="/com/castorama/search/droplet/AjouterTableRenderer">
                      ...
                      <dsp:oparam name="outputTableStart">
                        ...
                        <style type="text/css">
                          .ajouterListForm${ajouterFacetId} {
                            width: ${width}px;
                          }
                        </style>
                      </dsp:oparam>
                        ...
                        <div class="iOpt" style="width: ${ajouterColumnWidth}px">
                          <dsp:droplet name="/atg/dynamo/droplet/Switch">
                            ...
                            <dsp:oparam name="true"> 
                              <input type="checkbox" name="ajouter_${ajouterFacetId}" id="ajouter_${facetValue}" value="${facetValue}" checked
                                     onclick="javascript:addOrRemoveFacetValue(this,'${facetValue}');"/>
                              
                            </dsp:oparam>
                            <dsp:oparam name="false">
                              <input type="checkbox" name="ajouter_${ajouterFacetId}" id="ajouter_${facetValue}" value="${facetValue}"
                                     onclick="javascript:addOrRemoveFacetValue(this,'${facetValue}');"/>
                    ...
                    <script language="javascript">
                        function addOrRemoveFacetValue(_obj, _str){
                            if (document.location.search.match(/trail=/)) {
                                actionContainerVar = "&trail=" + processTRail(_obj, _str, "|", actionContainerVar, rootFacet);
                              } else {
                                actionContainerVar = "";
                              }
                            
                            if (rootFacet.length == 0){
                                rootFacet = getRootFacet(inputsFacetValues);
                            }
                            inputsFacetValues = processTRail(_obj, _str, "_", inputsFacetValues, rootFacet);
                            inputsFacetValues = inputsFacetValues.replace(/:/g,'/');
                            inputsFacetValues = inputsFacetValues.replace(/\|/g,'_');
                            var _formAction;
                            if (inputsFacetValues.length != 0) {
                                _formAction = (actionForForm+"/"+inputsFacetValues+inputsPageUrlForFacets+"${actionContainerVar}"+actionContainerVar).replace(/\s/g,'+');
                            } else {
                                _formAction = (actionForForm+inputsPageUrlForFacets+"${actionContainerVar}"+actionContainerVar).replace(/\s/g,'+');
                            }
                            $(_obj).parents('form')[0].action=_formAction;
                        }
