...
<div class="stockAvailabilityBody">
    <div class="stockAvailabilityHeader">
        Supervision du flux Stock
    </div>
    <div class="stockAvailabilityContent">
        <c:choose>
            <c:when test="${supervisionManager.dayDataStock}">
                <div class="stockAvailabilityContentBlock">
                    ...
                    <table class="stockAvailabilityTable">
                        ...
                        <c:forEach var="record" items="${syncJournalRecords}" varStatus="st">
                            ...
                            <tr class="<c:out value="${trclass}"/>">
                                ...
                                <td>
                                    <c:if test="${record.stockStatus != 3}">
                                        <c:out value="${record.stockStatus}"/>
                                    </c:if>
                                </td>
                                <td>
                                    <c:if test="${record.stockStatus == 0}">
                                        OK
                                        <c:if test="${st.index == 0}">
                                            <c:set var="totalResultStock" scope="page" value="OK"/>
                                        </c:if>
                                    </c:if>
                                    <c:if test="${record.stockStatus == 1}">
                                        KO
                                        <c:set var="totalResultStock" scope="page" value="KO"/>
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
                    ...
                    <div class="supervisionFooter">
                        ...
                        <div class="result">
                            RESULTAT : <c:out value="${totalResultStock}"/>
                        </div>
                    </div>
                </div>
            </c:when>
