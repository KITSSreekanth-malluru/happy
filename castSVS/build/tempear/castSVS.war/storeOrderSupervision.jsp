...
<head>
    ...
    <title>Supervision du Flux Visu de Commande </title>
    <link rel="stylesheet" type="text/css" href="css/stock.css"/>
</head>
<body>
<div class="stockAvailabilityBody">
    <div class="stockAvailabilityHeader">
        Supervision du flux Commande
    </div>
    <div class="stockAvailabilityContent">
        <c:choose>
            <c:when test="${supervisionManager.dayDataOrder}">
                <div class="stockAvailabilityContentBlock">
                    ...
                    <table class="stockAvailabilityTable">
                        <tr class="stockAvailabilityTableHeaderRow">
                            <td>Store ID</td>
                            <td>Date derniere extraction</td>
                            <td>Orders (nbr)</td>
                            <td>Status</td>
                            <td>Result</td>
                        </tr>
                        ...
                        <c:forEach var="record" items="${syncJournalRecords}" varStatus="st">
                            ...
                            <tr class="<c:out value="${trclass}"/>">
                                ...
                                <td>
                                    <c:out value="${record.ordersCount}"/>
                                </td>
                                <td>
                                    <c:if test="${record.orderStatus != 3}">
                                        <c:out value="${record.orderStatus}"/>
                                    </c:if>
                                </td>
                                <td>
                                    <c:if test="${record.orderStatus == 0}">
                                        OK
                                        <c:if test="${st.index == 0}">
                                            <c:set var="totalResultOrder" scope="page" value="OK"/>
                                        </c:if>
                                    </c:if>
                                    <c:if test="${record.orderStatus == 1}">
                                        KO
                                        <c:set var="totalResultOrder" scope="page" value="KO"/>
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
                    ...
                    <div class="supervisionFooter">
                        ...
                        <div class="result">
                            RESULTAT : <c:out value="${totalResultOrder}"/>
                        </div>
                    </div>
                </div>
            </c:when>
