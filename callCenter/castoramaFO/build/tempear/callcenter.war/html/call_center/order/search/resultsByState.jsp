...
<TABLE cellpadding="0"  cellspacing="0" border=1 bordercolor="#CC0033" width=450 align=center>
   ...
   <dsp:droplet name="/atg/dynamo/droplet/TableRange">
    	<dsp:param name="array" bean="OrderFinderByState.resultatRecherche"/>
  ...
  <c:choose>
    <c:when test="${numOrder < 400000000}">
					<A href="<%=request.getContextPath()%>/html/call_center/order/show/order_migr.jsp?id=<dsp:valueof param="element.repositoryId"/>">
						<dsp:valueof param="element.repositoryId"/>
					</A>
    </c:when>
    <c:otherwise> 
					<A href="<%=request.getContextPath()%>/html/call_center/order/show/order.jsp?id=<dsp:valueof param="element.repositoryId"/>">
						<dsp:valueof param="element.repositoryId"/>
					</A>
     </c:otherwise> 
  </c:choose>
				...
				<dsp:include page="../../common/fragmentEtat.jsp"><dsp:param name="state" param="etat"/></dsp:include>
				</TD>
		</TR>
  	  </dsp:oparam>
  	  <dsp:oparam name="empty">
